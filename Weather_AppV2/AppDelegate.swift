...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    func application(_ application: UIApplication, didFinishLaunchingWithOptions launchOptions: [UIApplicationLaunchOptionsKey: Any]?) -> Bool {
        GMSPlacesClient.provideAPIKey(apiKeyClass.google_key)
    ...
    func handleShortcutItem(withShortcutItem item: UIApplicationShortcutItem) -> Bool {
        ...
        if let type = ShortcutIdentifier(rawValue: shortcutType) {
            switch type {
            case .First:
                search = true
                return true
    ...
    func applicationDidBecomeActive(_ application: UIApplication) {
        ...
        if(search) {
            if let vc = self.window?.rootViewController {
                ...
                print(vc.childViewControllers[0])
                autocompleteController.delegate = vc.childViewControllers[0] as? GMSAutocompleteViewControllerDelegate
